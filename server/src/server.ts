...
import * as util from 'node:util'
import { getCustomCompletions } from './custom'
...
async function runCompiler (
  ...
  try {
    ...
    const output = await exec(`${settings.nushellExecutablePath} ${usingFlags} ${script_path_flag} ${tmp.name}`, {
...
async function getDefinition (
  ...
  output = (async (): Promise<CompletionItem[ 'documentation' ]> => {
    try {
      ...
      connection.console.error(`Encountered error trying to get definition, error: ${error}`)
    }
  })()
    .then((output) => {
      connection.console.info(`Got def for text ${text}: ${JSON.stringify(output)}`)
      return output
    })
    .catch((error) => {
      connection.console.error(`Encountered error trying to get def for ${text}, ${error}`)
      return undefined
    })
...
connection.onCompletion(async (request: TextDocumentPositionParams): Promise<CompletionItem[]> => {
  return await durationLogWrapper(`onCompletion`, async () => {
    ...
    const document = documents.get(request.textDocument.uri)
    const settings = await getDocumentSettings(request.textDocument.uri)
    const customs = await getCustomCompletions(settings, tmpFile)
        ...
        try {
          ...
          for (const completion of obj.completions) {
            const completionItem = (async () => {
              const cached = customs.get(completion)
              if (cached) {
              ...
              const item: CompletionItem = {
                ...
                data: index,
                documentation: await getDefinition(completion, request.textDocument),
              }
          ...
          connection.console.error(`Encountered error trying to JSON.parse onCompletion line ${line}: ${error}`)
        }
      }
    }
    ...
    const mappings = await Promise.all(definitions.values()).then((x) => x.filter((x) => x !== undefined))
